...
export const StudentPortalView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner: Student Academic Identity & Experience Preview */}
      <div className="bg-gradient-to-r from-emerald-800 via-teal-900 to-slate-900 rounded-3xl p-5 sm:p-6 lg:p-7 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 relative z-10">
          ...
          <div className="flex items-start sm:items-center space-x-3.5 sm:space-x-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                ...
                <span className="text-teal-400/60 hidden sm:inline">•</span>
                <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full border flex items-center gap-1.5 shadow-2xs ${
                  ...
                  <span className={`w-1.5 h-1.5 rounded-full ${
                    stats.status === 'On Track' ? 'bg-emerald-300 animate-pulse' : stats.status === 'Needs Attention' ? 'bg-amber-300' : 'bg-rose-300'
                  }`} />
          ...
          {/* Right: Sleek Quick Profile Switcher */}
          <div className="bg-black/20 backdrop-blur-md p-3 sm:p-3.5 rounded-2xl border border-white/15 shadow-sm flex flex-col gap-2 shrink-0 lg:w-80 self-stretch lg:self-auto">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-emerald-200 uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                Switch Student
              </span>
              <span className="text-[10px] text-emerald-300/80 font-semibold bg-white/10 px-2 py-0.5 rounded-full">
                Preview Mode
              </span>
            </div>
            <div className="relative">
              <select
                ...
                className="w-full bg-slate-900/90 hover:bg-slate-900 text-white text-xs font-semibold rounded-xl pl-3 pr-8 py-2 border border-emerald-500/40 focus:outline-none focus:ring-2 focus:ring-emerald-400 cursor-pointer appearance-none transition-all"
              >
                {students.map(s => {
                  const sStat = studentStats.find(st => st.student.id === s.id);
                  return (
                    <option key={s.id} value={s.id} className="bg-slate-900 text-white py-1">
                      {s.name} ({s.group || 'Grp 1'}) — {sStat ? `${sStat.attendancePercentage}% • ${sStat.status}` : s.currentLevel}
                    </option>
                  );
                })}
              </select>
              <ChevronDown className="w-4 h-4 text-emerald-300/80 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" />
            </div>
            <div className="flex items-center gap-1 overflow-x-auto no-scrollbar pt-0.5">
              <span className="text-[10px] text-emerald-200/70 shrink-0 font-medium mr-0.5">Quick:</span>
              {quickProfiles.map(p => {
                ...
                return (
                  <button
                    ...
                    className={`text-[10px] px-2 py-0.5 rounded-lg font-medium transition-all shrink-0 cursor-pointer ${
                      isSelected
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mt-6 pt-5 border-t border-white/15 relative z-10">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-3.5 sm:px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer ${
                    isActive
                ...
                >
                  ...
                  {tab.badge && (
                    <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-emerald-500 text-white font-bold shrink-0">
                      {tab.badge}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center gap-2 shrink-0 self-start md:self-auto">
            <button
              ...
              className="px-3 py-2 rounded-xl text-xs font-bold bg-white/15 hover:bg-white/25 text-white transition-all flex items-center gap-1.5 border border-white/20 shadow-xs cursor-pointer active:scale-95"
            >
              <FileText className="w-3.5 h-3.5 text-teal-200 shrink-0" />
              <span className="whitespace-nowrap">Official Transcript</span>
            </button>
            <button
              type="button"
              onClick={() => {
              ...
              className="px-3 py-2 rounded-xl text-xs font-bold bg-amber-400/20 hover:bg-amber-400/30 text-amber-200 transition-all flex items-center gap-1.5 border border-amber-300/40 shadow-xs cursor-pointer active:scale-95"
            >
              <Award className="w-3.5 h-3.5 text-amber-300 shrink-0" />
              <span className="whitespace-nowrap">Certificate of Merit</span>
            </button>
          </div>
        </div>
      </div>
